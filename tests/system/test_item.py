    ...
    def setUp(self):
        ...
        with self.app() as client:
            with self.app_context():
                ...
                self.access_token = f'JWT {auth_token}' #save auth token as object property
    ...
    def test_put_update_item(self):
        with self.app() as client:
            with self.app_context():
                ...
                resp = client.put('/item/test', data={'price': 7.99, 'store_id': 1})

                self.assertEqual(resp.status_code, 200)
                self.assertEqual(ItemModel.find_by_name('test').price, 7.99)
                self.assertDictEqual({'name': 'test', 'price': 7.99},
                                     json.loads(resp.data))
    ...
    def test_item_list(self):
        with self.app() as client:
            with self.app_context():
                StoreModel('test').save_to_db()
                ItemModel('test', 5.99, 1).save_to_db()
                ItemModel('test1', 7.99, 1).save_to_db()

                resp = client.get('/items')

                self.assertDictEqual({'items': [{'name': 'test', 'price': 5.99},
                                                {'name': 'test1', 'price': 7.99}]},
                                    json.loads(resp.data))